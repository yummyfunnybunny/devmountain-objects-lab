...
//console.log(deleteTheBigNumbers.first);
// for (const num in Object.entries(deleteTheBigNumbers)) {
//   console.log(num);
// }
...
//Code Here
function usCanadaBorder(latLon) {
  const [latitude, longitude] = latLon;

  if (latitude === 49) {
    if (longitude >= -123 && longitude <= -95) {
      return true;
    }
  }
  return false;
...
function recordCleaner() {
  for (const atFaultRecord of myCar.accidents) {
    //console.log(atFaultRecord);
    atFaultRecord.atFaultForAccident = false;
  }
}
